...
> **핵심 원칙: 로컬 우선 (Local-first)**
> 미로그인 시 로컬 저장으로 즉시 사용 가능. 로그인은 선택사항.
> 로그인 시 로컬 데이터를 Firestore로 자동 동기화(마이그레이션).

```
인증 시스템을 "로그인 선택" 구조로 구현해줘.

- 첫 실행 시 로그인 없이 바로 캘린더 사용 가능 (현재 로컬 저장 유지)
- Settings 화면에서 "Sign In" 버튼으로 로그인 가능
- 로그인 방법: 이메일+비밀번호, Google 소셜 로그인
- 로그인 시 기존 로컬 데이터를 Firestore로 자동 마이그레이션
- 로그인 후에는 Firestore 실시간 동기화 활성화
- 워크스페이스 기능은 로그인 후에만 사용 가능
```
