...
| 로컬 우선 모드 | 미로그인 시 로컬 저장으로 즉시 사용 가능. 로그인은 선택사항. 로그인 시 로컬 데이터를 Firestore로 자동 동기화(마이그레이션). | P0 |
| 워크스페이스 생성 | 팀/조직 단위 워크스페이스 (로그인 후 사용 가능) | P0 |
| 워크스페이스 전환 | 사이드 메뉴 상단 드롭다운 (로그인 후 사용 가능) | P0 |
